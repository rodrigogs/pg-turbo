...
function makeJob(index: number): ChunkJob {
  ...
  return {
    table, chunk: { index: 0, file: `data/public.table_${index}/chunk_0000.copy.lz4` },
...
describe('runWorkerPool', () => {
  it('processes all jobs with single worker', async () => {
    ...
    const results = await runWorkerPool({ jobs, workerCount: 1, task, onProgress: vi.fn(), maxRetries: 3, isResumable: () => false })
  ...
  it('processes jobs in parallel', async () => {
    ...
    const task = vi.fn().mockImplementation(async () => {
      running.push(1); maxConcurrent = Math.max(maxConcurrent, running.length)
      await new Promise(r => setTimeout(r, 10)); running.pop()
  ...
  it('marks job failed after exhausting retries', async () => {
    ...
    const results = await runWorkerPool({ jobs, workerCount: 1, task, onProgress: vi.fn(), maxRetries: 2, isResumable: () => false })
    expect(results).toHaveLength(1)
  ...
  it('skips resumable jobs', async () => {
    ...
    const results = await runWorkerPool({
      jobs, workerCount: 1, task, onProgress: vi.fn(), maxRetries: 3,
