...
import { sanitizeConnectionString, extractDbName, cleanConnectionString, appendKeepaliveParams } from '../../src/core/connection.js'
...
describe('sanitizeConnectionString', () => {
  ...
  it('strips query params', () => {
    expect(sanitizeConnectionString('postgresql://user:pass@host/db?sslmode=require')).toBe('postgresql://user:***@host/db')
  })
})

describe('extractDbName', () => {
  it('extracts database name', () => { expect(extractDbName('postgresql://user:pass@host/mydb')).toBe('mydb') })
  it('strips query params', () => { expect(extractDbName('postgresql://user:pass@host/mydb?sslmode=require')).toBe('mydb') })
  it('handles port', () => { expect(extractDbName('postgresql://user:pass@host:5432/mydb')).toBe('mydb') })
})

describe('cleanConnectionString', () => {
  it('strips GUI params but keeps sslmode', () => {
    expect(cleanConnectionString('postgresql://u:p@h/db?statusColor=red&sslmode=require&env=staging')).toBe('postgresql://u:p@h/db?sslmode=require')
  })
