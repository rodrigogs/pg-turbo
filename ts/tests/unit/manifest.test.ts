...
const SAMPLE_MANIFEST: DumpManifest = {
  version: 1, tool: 'pg-resilient', createdAt: '2026-03-31T05:00:00.000Z',
  pgVersion: '16.2', database: 'testdb', snapshotId: '00000003-1B', compression: 'lz4',
  options: { schemaFilter: null, splitThresholdBytes: 1_073_741_824, jobs: 4 },
  tables: [{
    schema: 'public', name: 'users', oid: 16385, relkind: 'r',
    estimatedBytes: 1024, estimatedRows: 10, pkColumn: 'id', pkType: 'int8',
    chunkStrategy: 'none', columns: ['id', 'name'], generatedColumns: [],
...
describe('manifest', () => {
  let tmpDir: string
  beforeEach(() => { tmpDir = mkdtempSync(join(tmpdir(), 'pgr-test-')) })
