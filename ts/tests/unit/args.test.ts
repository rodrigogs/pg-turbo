...
describe('parseSize', () => {
  it('parses plain bytes', () => { expect(parseSize('1024')).toBe(1024) })
  it('parses KB', () => { expect(parseSize('10KB')).toBe(10 * 1024) })
  it('parses MB', () => { expect(parseSize('256MB')).toBe(256 * 1024 * 1024) })
  it('parses GB', () => { expect(parseSize('1GB')).toBe(1024 * 1024 * 1024) })
  it('throws on invalid input', () => { expect(() => parseSize('abc')).toThrow() })
})
...
describe('parseDumpArgs', () => {
  ...
  it('parses optional flags', () => {
    const opts = parseDumpArgs(['-d', 'pg://h/db', '--output', './out', '-n', 'public', '-j', '8', '--dry-run', '--no-snapshot', '--split-threshold', '512MB'])
  ...
  it('captures passthrough args after --', () => {
    const opts = parseDumpArgs(['-d', 'pg://h/db', '--output', './out', '--', '--no-comments', '--lock-wait-timeout=300'])
