...
async function timeIt(label: string, fn: () => Promise<void> | void): Promise<number> {
...
async function main() {
  ...
  // Show DB size
  const dbSize = execSync(`psql "${CS}" -tAc "SELECT pg_database_size(current_database())"`)
    .toString().trim()
  console.log(`Database size: ${(parseInt(dbSize) / 1024 / 1024).toFixed(1)} MB\n`)

  // Show table sizes
  execSync(`psql "${CS}" -c "SELECT schemaname || '.' || relname AS table, pg_size_pretty(pg_total_relation_size(schemaname || '.' || relname)) AS size FROM pg_stat_user_tables ORDER BY pg_total_relation_size(schemaname || '.' || relname) DESC"`)
  ...
  // Benchmark 1: pg_dump custom format (single process)
  {
    ...
    const ms = await timeIt('pg_dump -Fc', () => {
      execFileSync('pg_dump', [CS, '--format=custom', '--no-owner', '--no-privileges', '-f', outFile], { stdio: 'pipe' })
    })
  ...
  // Benchmark 2: pg_dump directory format with -j 4
  {
    ...
    const ms = await timeIt('pg_dump -Fd -j4', () => {
      execFileSync('pg_dump', [CS, '--format=directory', '--jobs=4', '--no-owner', '--no-privileges', '-f', outDir], { stdio: 'pipe' })
    })
    const size = dirSize(dir)
  ...
  // Benchmark 3: pg-resilient with 1 worker
  {
    ...
    const ms = await timeIt('pg-resilient -j1', async () => {
      await runDump({
        dbname: CS, output: dir, jobs: 1, splitThreshold: 1024 * 1024 * 1024,
        maxChunksPerTable: 32, retries: 1, retryDelay: 1,
        noSnapshot: false, dryRun: false, pgDumpArgs: [],
      })
    })
    const size = dirSize(dir)
  ...
  // Benchmark 4: pg-resilient with 4 workers
  {
    ...
    const ms = await timeIt('pg-resilient -j4', async () => {
      await runDump({
        dbname: CS, output: dir, jobs: 4, splitThreshold: 1024 * 1024 * 1024,
        maxChunksPerTable: 32, retries: 1, retryDelay: 1,
        noSnapshot: false, dryRun: false, pgDumpArgs: [],
      })
    })
  ...
  for (const r of results) {
    ...
    console.log(`| ${r.label.padEnd(25)} | ${secs.padStart(7)}s | ${sizeMB.padStart(8)} MB | ${speedMBs.padStart(6)} MB/s |`)
  }

  const baseline = results[0]!.timeMs
  console.log('\n=== RELATIVE PERFORMANCE ===\n')
  for (const r of results) {
    const ratio = (baseline / r.timeMs).toFixed(2)
    console.log(`${r.label.padEnd(25)} ${ratio}x ${r.timeMs < baseline ? 'FASTER' : r.timeMs > baseline ? 'slower' : '(baseline)'}`)
  }
}
