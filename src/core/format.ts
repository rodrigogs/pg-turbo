...
export function computeBar(current: number, total: number, width: number): BarMetrics {
  const pct = total > 0 ? Math.min(Math.floor((current * 100) / total), 100) : 0
  const filledCount = total > 0 ? Math.max(0, Math.min(Math.floor((current * width) / total), width)) : 0
  return {
    pct,
...
export function progressBar(current: number, total: number, width: number = 30): string {
  ...
  const status = total > 1024 ? `(${humanSize(current)} / ${humanSize(total)})` : `(${current}/${total})`
  return `[${filled}${empty}] ${pct.toString().padStart(3)}% ${status}`
}
